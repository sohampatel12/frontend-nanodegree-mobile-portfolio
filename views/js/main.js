...
var resizePizzas = function(size) {
	...
	function changePizzaSizes(size) {
		// Changed the jQuery part out of the for loop as it is not required everytime in the loop.
		var pizzaElements = document.getElementsByClassName("randomPizzaContainer");
		// The fucntion determineDx is also moved out of the for loop for faster processing 
...
function updatePositions() {
	...
	// Created an array that stores the 5 phases used instead of calculating everytime in the for loop 
	phases = [];
	for (var j = 0; j < 5; j++) {
		phases.push(Math.sin((document.body.scrollTop / 1250) + (j % 5)));
	}
	// The original loop calculating the new positions of each pizza element
...
document.addEventListener('DOMContentLoaded', function() {
	var cols = 8;
	var s = 256;
	// Dynamically calculates pizzas required in a page.
	var rows = Math.round(window.screen.height / s);
	var pizzaCount = rows * cols;
	// Replaced querySelector by getElementById and moved it out of the for loop.
